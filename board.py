# from pydlgo.config import BOARD_SIZE, KOMI, INPUT_CHANNELS, PAST_MOVES
...
class Board(object):
    ...
    def text_to_vertex(self, text):
        ...
        vtx = self.index_to_vertex(idx)
        # DEBUG
        # print("vertex={0}".format(vtx))
        return vtx

    def sgf_to_vertex(self, text):
        # 新增的功能
        # sgf move to vertex
        # return nothing if text is illegal.
    ...
    def get_last_move(self):
        ...
        if self.last_move == NULL_VERTEX:
            return []
            
        return [self.get_x(self.last_move), self.get_y(self.last_move)]
